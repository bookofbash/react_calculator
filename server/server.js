...
/* 
- Function to be used as a allback to our http.createServer method
- It handeles incoming requests and sends the response
*/

function requestHandler(request, response) {
    //resolve the path to the requested resource and assign in to a variale
    let requestedResource = path.join(
        ...
        url.parse(request.url).pathname // path to resource reqested by client
    );
    ...
    fs.exists(requestedResource, function(exists) {
        //check if file doesn't exist and return a 404 satus (File not found)
        if(!exists) {
        ...
        fs.readFile(
            ...
            function(err, file) { //call back function to handel end of file read
            
            //if an error occured while reading the file, send error message
            ...
            //Helper object to map requested content types (extension) to respondse
            // mime types
            ...
            const headers = {};
            //get the content type using thr requested resource file extension
            const contentType = contentTypesByExtension[
                path.extname(requestedResource)
            ];

            // if the requested resours maps to any of our content type extension
